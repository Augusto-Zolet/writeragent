...
class SearchEmbeddings(ToolBase):
    ...
    def execute(self, ctx: ToolContext, **kwargs: Any) -> dict[str, Any]:
        ...
        def _run() -> dict[str, Any]:
            ...
            folder_key, db_path, listing_or_err = resolve_index_context(ctx.ctx, ctx.doc)
            if folder_key is None or db_path is None:
                return {"status": "error", "message": listing_or_err}

            if index_is_empty(db_path):
            ...
            try:
                result = knn_search(ctx.ctx, str(db_path), str(query), k, model=model)
