...
OpenAI’s own host is in `PROVIDERS` and Quick Setup but **not** in the Settings dropdown. If someone pastes `api.openai.com`, **we send today:** `POST /v1/images/generations` JSON with top-level `image_url` (not a documented generations field). **Docs say:** create is `POST /v1/images/generations` JSON (`prompt`, `model`, `n`, `size`, … — no source image). Edit is `POST /v1/images/edits` for GPT Image and `dall-e-2` (not `dall-e-3`): either `multipart/form-data` (`image` file, optional `mask`) or `application/json` with **`images`** (array of `{image_url}` or `{file_id}`, up to 16 on GPT Image) plus optional `mask`, `input_fidelity` (`high`/`low`), `prompt`, `model` ([Create image edit](https://developers.openai.com/api/reference/resources/images/methods/edit), [Image generation guide](https://developers.openai.com/api/docs/guides/image-generation)). Default model on that route is currently a GPT Image id (docs list `gpt-image-1.5` / `gpt-image-2` / `gpt-image-2.5-*`). There is **no** `strength` field; closest is `input_fidelity`.

### Same failure mode as OpenRouter

The OpenRouter bug was: **HTTP 200 + a new image**, because the field name was wrong. Together / Grok / Gemini image / Ollama now send a documented edit field (Imagen raises instead of generating). That pattern is still possible wherever we send top-level `image_url` on `/images/generations` and the server ignores unknown JSON keys: **Z.ai** (create-only, documented fields are `model`/`prompt`/`quality`/`size`/`user_id`), **pasted `api.openai.com`** (generations has no source-image field; edit is a different path), **self-hosted Visual GenAI NIM** (edit field is `image`, not `image_url`), and any **custom OpenAI-compat** that implements create only.
...
|----------|--------|---------------------------|-----|
| **OpenAI** (`api.openai.com`, not in dropdown) | **Implement** in `OpenAIShim` when host is OpenAI | **Create:** keep `POST {api_path}/images/generations` JSON (`prompt`, `model`, `n`, `size`, `response_format: b64_json`). **Edit:** `POST {api_path}/images/edits`, `Content-Type: application/json`, `{model, prompt, n, images: [{image_url: "<data URL>"}]}`. Optional later: `mask`, `input_fidelity`. Multipart `image=@file` is equivalent but heavier; JSON `images[].image_url` matches our in-memory base64. | Official Images API. Today’s top-level `image_url` on **generations** is not a documented field → OpenRouter-class 200+new-image if the server ignores it, or 400 if it rejects unknown keys. `dall-e-3` is generations-only; GPT Image + `dall-e-2` support edits. |
