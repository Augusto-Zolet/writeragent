...
- **OpenRouter chat / `modalities: ["image"]` path** (Gemini and other multimodal image models): adds `image_config.aspect_ratio` and `image_config.image_size`. That path previously sent no size/aspect at all, so Square / 2048 in the sidebar never reached Gemini.
