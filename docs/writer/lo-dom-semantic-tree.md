...
*   **Embedded Draw Shapes:** Writer documents contain a hidden "draw page" layer (`doc.getDrawPage()`). Page-anchored shapes use `AnchorPageNo`. Paragraph/character-anchored shapes use the same view-cursor page check as tables and images (`gotoRange(anchor)` + `vc.getPage()`). Do not classify shapes by cloning the view cursor through the body `XText` after `jumpToEndOfPage` — that raises UNO `RuntimeException` when the page end sits in a table cell or frame. `lockControllers` is applied only after an unlocked hop to `doc.getText().getStart()`; locking while the cursor is still in a cell makes `gotoRange`/`getPage` fail silently (`jumpToPage` is a no-op on the same page). Unlock before restoring a nested-cell cursor. After that hop, `getPage()` is valid at body text, but a locked `gotoRange` to a table/frame `getAnchor()` leaves `getPage()` at 0 (stale layout, not an empty page) — unlock, hop again, relock. The snapshot still includes geometry, text, and names for flowcharts and diagrams in the text report.
