...
| Resize / width sync with Send–Clear row | [`panel_resize.py`](../plugin/chatbot/panel_resize.py) sizes the hidden `response` placeholder; [`sync_rich_control_bounds`](../plugin/chatbot/rich_text_control.py) insets `response_rich` inside `last_response_rect` (width matches the placeholder; early init without a layout rect caps to the live Clear button) |
