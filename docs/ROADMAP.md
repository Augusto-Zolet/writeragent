...
| 2 | **Missing API key → fake 401** — [`llm_client.py`](../plugin/framework/client/llm_client.py) `_resolve_auth`, [`auth.py`](../plugin/framework/client/auth.py) `resolve_auth_for_config` | **Shipped:** `_resolve_auth` no longer swallows `AuthError` into `{}`. Hosted missing keys raise `missing_api_key` before HTTP; Ollama / `header_style=none` and custom empty keys still omit auth headers. |
