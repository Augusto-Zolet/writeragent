...
| Final | **`ty check`** reports **no errors** for the configured include set. **`make typecheck`** runs **`ruff-for-build`**, **`ty`**, **`mypy`**, **`basedpyright`**, and **`pyspector`** in sequence; **`make test`** runs those five, then pytest and LO tests (types and SAST before tests). **`make release`** calls **`make test`** first, then the release bundle (see **`Makefile`**). |
