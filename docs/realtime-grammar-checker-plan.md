...
- **Chunking**: To avoid hitting LLM output token limits (which can cause a full batch to fail), the worker splits large sentence lists into smaller chunks (default **`GRAMMAR_BATCH_MAX_SENTENCES = 4`**). Each chunk is a separate LLM request.
- **Force Single Sentence**: A configuration flag `doc.grammar_proofreader_force_single_sentence` (default **True**) can disable batching entirely, forcing every sentence into its own request.
...
- **Paragraph Batching (Chunked)**: Grouped sentences from the same paragraph/context are sent to the LLM in batches. To ensure the JSON response fits within model output token limits and to minimize latency, the system **chunks** these batches (default **4 sentences per request**, defined by **`GRAMMAR_BATCH_MAX_SIZE`** in [`grammar_proofread_locale.py`](../plugin/writer/locale/grammar_proofread_locale.py)). This significantly reduces latency and token overhead during full-paragraph checks or document loads compared to individual requests.
