...
"""Tests for plugin.scripting.embeddings_ods_extract."""
...
from plugin.scripting import embeddings_ods_extract
...
def test_extract_calc_rows_from_ods(tmp_path: Path):
    ...
    rows = embeddings_ods_extract.extract_calc_rows(str(ods))
    assert len(rows) == 1
...
def test_extract_calc_rows_missing_pandas(tmp_path: Path):
    ...
    with patch("builtins.__import__", side_effect=_fake_import):
        assert embeddings_ods_extract.extract_calc_rows(str(ods)) == []
