...
"""Tests for plugin.scripting.embeddings_folder_maintain."""
...
from plugin.scripting import embeddings_folder_maintain
...
def test_maintain_cold_calls_heartbeat_and_ingest(tmp_path: Path):
    ...
    with patch("plugin.scripting.embeddings_folder_maintain.index_is_empty", return_value=True):
        with patch("plugin.scripting.embeddings_folder_maintain.needs_cold_rebuild", return_value=True):
            with patch("plugin.scripting.embeddings_folder_maintain.clear_folder_cache") as clear_mock:
                with patch(
                    "plugin.scripting.embeddings_folder_maintain._ingest_rows",
...
def test_maintain_incremental_skips_fresh_file(tmp_path: Path):
    ...
    from plugin.doc.embeddings_cache import mark_file_indexed
    from plugin.doc.embeddings_fs import content_hash, path_to_file_url
    ...
    with patch("plugin.scripting.embeddings_folder_maintain._ingest_rows") as ingest_mock:
        result = embeddings_folder_maintain.maintain_folder_index(
            str(tmp_path),
            embedding_model="all-MiniLM-L6-v2",
            mode="incremental",
