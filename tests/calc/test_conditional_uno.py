...
def test_calc_conditional_formatting(ctx, doc):
    # Write some numbers
    _execute_calc_tool(doc, ctx, "write_formula_range", {"range_name": "C20:C22", "formula_or_values": [
        [10],
        [20],
        [30]
    ]})
    ...
    res_list = _execute_calc_tool(doc, ctx, "list_conditional_formats", {"range_name": "C20:C22"})
    ...
    res_clear = _execute_calc_tool(doc, ctx, "remove_conditional_formats", {"range_name": "C20:C22"})
    assert res_clear.get("status") == "ok", f"remove_conditional_formats (clear) failed: {res_clear}"

    # 4. Verify cleared
    res_list_after = _execute_calc_tool(doc, ctx, "list_conditional_formats", {"range_name": "C20:C22"})
    ...
    res_remove_idx = _execute_calc_tool(doc, ctx, "remove_conditional_formats", {"range_name": "C20:C22", "rule_index": 0})
    assert res_remove_idx.get("status") == "ok", f"remove_conditional_formats (index) failed: {res_remove_idx}"

    # 7. Verify only 1 rule remains
    res_list_final = _execute_calc_tool(doc, ctx, "list_conditional_formats", {"range_name": "C20:C22"})
    ...
    _execute_calc_tool(doc, ctx, "remove_conditional_formats", {"range_name": "C20:C22"})
    _execute_calc_tool(doc, ctx, "write_formula_range", {"range_name": "C20:C22", "formula_or_values": [[5], [10], [15]]})
    ...
    res_lb = _execute_calc_tool(doc, ctx, "list_conditional_formats", {"range_name": "C20:C22"})
    br = res_lb.get("rules", [])
    ...
    _execute_calc_tool(doc, ctx, "remove_conditional_formats", {"range_name": "E20:E22"})
    _execute_calc_tool(doc, ctx, "write_formula_range", {"range_name": "E20:E22", "formula_or_values": [[1], [1], [2]]})
    ...
    res_ld = _execute_calc_tool(doc, ctx, "list_conditional_formats", {"range_name": "E20:E22"})
    dr = res_ld.get("rules", [])
    assert len(dr) == 1, dr
